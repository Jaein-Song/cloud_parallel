...
!1.   read unfiltered (D) cfradial file
    ...
    if (filei.eq.1) ALLOCATE(RefvI(ftlen,binlen),VelvI(ftlen,binlen),SpWvI(ftlen,binlen),SNRvI(ftlen,binlen),LDRaI(ftlen,binlen))
    ALLOCATE(frequencyarrI(frqlen),PRTI(ftlen_NC),pulse_widtharrI(ftlen_NC),prt_ratioarrI(ftlen_NC),nyq_VelarrI(ftlen_NC),n_samplesarrI(ftlen_NC),WavePRF(ftlen_NC))
